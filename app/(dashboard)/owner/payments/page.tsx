  ...
  const loadData = async () => {
    try {
      ...
      const defaultFin = `${year}-${month}-${String(lastDay).padStart(2, '0')}`;
        ...
        const paiementsResponse = await ComptabiliteService.getPaiements({ page_size: 100 });
        const balanceResponse = await ComptabiliteService.getBalanceGlobale({
          periode_debut: defaultDebut,
          periode_fin: defaultFin,
        });
        const bauxResponse = await LocationsService.getBaux({});
      ...
      setBaux(bauxList.filter(b => b.statut === 'actif'));

      if (paiementsResponse.success && paiementsResponse.data) {
        const mappedPayments = paiementsResponse.data.map(p => {
          const uiResult = mapPaiementToUI(p);
          
          if (uiResult.tenantName === 'Locataire inconnu' || uiResult.propertyTitle === 'Bien inconnu') {
            const bailIdToMatch = p.bail_id || p.bail;
            const matchedBail = bauxList.find(b => b.id === bailIdToMatch);
            if (matchedBail) {
              uiResult.tenantName = matchedBail.locataire_nom || uiResult.tenantName;
              uiResult.propertyTitle = matchedBail.bien_adresse || uiResult.propertyTitle;
            }
          }
          return uiResult;
        });
            ...
            {baux.map((bail) => (
              <article key={bail.id} className="flex flex-col gap-2 rounded-xl border border-zinc-100 bg-zinc-50 p-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="text-xs text-zinc-500">Début: {bail.date_entree} · Fin: {bail.date_sortie}</p>
